...
function SharedService($rootScope) {
  ...
  shared.addFiles = (files) => {
    this.added_files = files;
  };

  shared.getAddedFiles = () => this.added_files;

  shared.hasAddedFiles = () =>
    Object.prototype.hasOwnProperty.call(this, "added_files");

  shared.resetAddedFiles = () => {
    delete this.added_files;
  };
  ...
  shared.changeSettings = (settings) => {
    ...
    AWS.config.update(
      Object.assign(settings.cred, { region: settings.region })
    );
        ...
        if (err1) {
          ...
          sts.getSessionToken(params, (err2, data2) => {
            if (err2) {
              ...
              $rootScope.$broadcast("broadcastChangeSettings", {
  ...
  shared.trashObjects = (bucket, keys) =>
    $rootScope.$broadcast("broadcastTrashObjects", { bucket, keys });

  shared.addFolder = (_bucket, _folder) =>
  ...
  shared.correctClockSkew = (Bucket) => {
    ...
    s3.headBucket({ Bucket }, (err, data) => {
      if (err) {
        ...
        DEBUG.log(
          "headBucket headers:",
          JSON.stringify(this.httpResponse.headers)
        );
...
function ViewController($scope, SharedService) {
    ...
    $scope.$apply(() => {
      ...
      if (args.viewprefix !== undefined && args.viewprefix !== null) {
        ...
        $.fn.dataTableExt.afnFiltering.push(
          ...
          (_o, d, _i) =>
            d[1] !== args.viewprefix && d[1].startsWith(args.viewprefix)
        );
  ...
  function makeS3FileDownloadLink(s3Key, href, text, download) {
    if (download) {
      return `<a data-s3="object" data-s3key="${s3Key}" href="${href}" download="${download}">${sanitizeString(
        text
      )}</a>`;
    }

    return `<a data-s3="folder" data-s3key="${s3Key}" href="${href}">${sanitizeString(
      text
    )}</a>`;
  }
  ...
  $scope.renderObject = (s3FileKey, _type, s3File) => {
    ...
    if (s3File.CommonPrefix) {
      ...
      if ($scope.view.settings.prefix) {
        return makeS3FileDownloadLink(
          encodedKey,
          href,
          prefix2folder(s3FileKey)
        );
      }
    ...
    return makeS3FileDownloadLink(
      encodedKey,
      href,
      fullpath2filename(s3FileKey),
      fullpath2filename(s3FileKey)
    );
  };

  $scope.renderFolder = (data, _type, full) =>
    full.CommonPrefix ? "" : fullpath2pathname(data);
  ...
  $scope.listobjectscb = (err, data) => {
    ...
    if (err) {
      ...
      $.each(data.CommonPrefixes, (index, value) => {
        $tb.DataTable().rows.add([{ CommonPrefix: true, Key: value.Prefix }]);
        count.objects++;
      });
  ...
  this.renderFolder = (data, type, full) =>
    $scope.renderFolder(data, type, full);
    ...
    document
      ...
      .forEach((checkbox) => {
        ...
        checkbox.checked = Boolean(
          $scope.view.keys_selected.find((e2) => e2.Key === data.Key)
        );
      });
  };
  ...
  $tb.DataTable({
    ...
    aoColumnDefs: [
      ...
      },
      {
        aTargets: [1],
        mData: "Key",
        mRender: this.renderObject,
        sType: "key",
      },
      {
        aTargets: [2],
        mData: "Key",
        mRender: this.renderFolder,
      },
      {
        aTargets: [3],
        mData: "LastModified",
        mRender: this.renderLastModified,
      },
      {
        aTargets: [4],
        mData: "LastModified",
        mRender: this.renderTimestamp,
      },
      {
        aTargets: [5],
        mData: this.dataSize,
  ...
  $("#select-all").on("click", () => {
    ...
    $tb
      ...
      .each((data) => {
        ...
        const checkbox =
          link && link.parentElement && link.parentElement.parentElement
            ? link.parentElement.parentElement.querySelector(
                "input[type='checkbox']"
              )
            : null;
...
function AddFolderController($scope, SharedService) {
  ...
  $scope.$on("broadcastChangeSettings", (e, args) => {
    DEBUG.log(
      "AddFolderController",
      "broadcast change settings bucket:",
      args.settings.bucket
    );
...
function SettingsController($scope, SharedService) {
  ...
  $scope.update = () => {
    ...
    $scope.settings.bucket =
      $scope.settings.selected_bucket || $scope.settings.entered_bucket;
    ...
    if ($scope.settings.entered_bucket) {
      ...
      if (
        $.inArray($scope.settings.entered_bucket, $scope.settings.buckets) ===
        -1
      ) {
...
function UploadController($scope, SharedService) {
  ...
  $scope.uploadFiles = (Bucket, prefix) => {
    ...
    $scope.upload.files.forEach((file, ii) => {
      ...
      const funcsend = (err, data) => {
        if (err) {
          ...
          if (err.code === "AccessDenied") {
            $(`#upload-td-${ii}`).html(
              '<span class="uploaderror">Access Denied</span>'
            );
          } else {
            DEBUG.log(JSON.stringify(err));
            $(`#upload-td-${ii}`).html(
              `<span class="uploaderror">Failed:&nbsp${err.code}</span>`
            );
            SharedService.showError(params, err);
          }
        } else {
      ...
      s3.upload(params).on("httpUploadProgress", funcprogress).send(funcsend);
    });
  };
    ...
    for (let ii = 0; ii < objects.length; ii++) {
        ...
        s3.listObjects(params, (err, data) => {
          if (err) {
            if (!recursion) {
              ...
              if (err.code === "AccessDenied") {
                $(`#trash-td-${ii}`).html(
                  '<span class="trasherror">Access Denied</span>'
                );
              } else {
                DEBUG.log(JSON.stringify(err));
                $(`#trash-td-${ii}`).html(
                  `<span class="trasherror">Failed:&nbsp${err.code}</span>`
                );
                SharedService.showError(params, err);
              }
            } else {
      ...
      s3.deleteObject(params, (err, _data) => {
        if (err) {
          ...
          if (!recursion) {
            $(`#trash-td-${ii}`).html(
              '<span class="trashdeleted">Deleted</span>'
            );
    ...
    for (let ii = 0; ii < args.keys.length; ii++) {
      ...
      const td = [
        ...
        $("<td>")
          .append(
            isfolder(obj.Key)
              ? prefix2folder(sanitizedS3Key)
              : fullpath2filename(sanitizedS3Key)
          )
          .attr("title", sanitizedS3Key),
        $("<td>").append(
          isfolder(obj.Key)
            ? prefix2parentfolder(sanitizedS3Key)
            : fullpath2pathname(sanitizedS3Key)
        ),
        $("<td>").append(
          isfolder(obj.Key) ? "" : moment(obj.LastModified).fromNow()
        ),
        $("<td>").append(
          obj.LastModified
